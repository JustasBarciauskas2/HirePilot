import { deleteVacancyOnBackend } from "@/lib/forward-vacancy";
import { resolveVacancyIdForPortalDelete } from "@/lib/public-jobs";
...
export async function DELETE(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  const fromQuery =
    ...
    null;
  const vacancyId =
    fromQuery ?? (await resolveVacancyIdForPortalDelete(ref)) ?? null;

  const backend = await deleteVacancyOnBackend(ref, idToken, { vacancyId });
