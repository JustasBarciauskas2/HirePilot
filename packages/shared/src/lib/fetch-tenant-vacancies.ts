...
import {
  ...
  type JobSizeBand,
  type JobSkill,
} from "@techrecruit/shared/data/job-types";
...
export function getResolvedVacanciesListUrl(tenantIdOverride?: string): string | null {
  const listBase = getBackendVacanciesListUrl();
  if (!listBase) return null;
  const tenantId = tenantIdOverride?.trim() || getTenantInstancePayload().id;
  try {
 ...
 */
export async function fetchTenantVacanciesResult(tenantIdOverride?: string): Promise<TenantVacanciesFetchResult> {
  const listBase = getBackendVacanciesListUrl();
