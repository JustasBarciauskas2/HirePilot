...
function DocumentProcessingOverlay({ fileName }: { fileName: string }) {
  return (
    <motion.div
      ...
      className="absolute inset-0 z-20 flex flex-col items-center justify-center rounded-2xl bg-white/88 px-6 py-10 shadow-[inset_0_0_0_1px_rgba(37,99,235,0.08)] backdrop-blur-md dark:bg-slate-900/92 dark:shadow-[inset_0_0_0_1px_rgba(56,189,248,0.1)]"
    >
      <div className="relative mx-auto h-32 w-28 overflow-hidden rounded-2xl bg-gradient-to-b from-white via-zinc-50/95 to-zinc-100/90 shadow-[0_12px_40px_-12px_rgba(37,99,235,0.25)] ring-1 ring-zinc-200/90 dark:from-slate-800/90 dark:via-slate-800/70 dark:to-slate-900/80 dark:ring-slate-600/40">
        <motion.div
          className="pointer-events-none absolute inset-0 h-full w-[45%] bg-gradient-to-r from-transparent via-[#2563EB]/20 to-transparent blur-[2px]"
          initial={{ x: "-100%" }}
          animate={{ x: "280%" }}
        ...
        >
          <FileText className="h-14 w-14 text-[#2563EB]" weight="duotone" aria-hidden />
        </motion.div>
      </div>

      <p className="mt-6 text-center font-display text-base font-semibold tracking-tight text-zinc-900 dark:text-slate-100">
        Working on your file
      </p>
      <p className="mt-1 max-w-[220px] text-center text-xs leading-relaxed text-zinc-500 dark:text-slate-400">
        Reading the document and pulling out the role details.
      </p>
      <p className="mt-3 max-w-full truncate text-center font-mono text-[11px] text-zinc-400 dark:text-slate-500" title={fileName}>
        {fileName}
      </p>
...
export function FileUploadWizard({ user, tenantId, onBack }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-[0_24px_60px_-28px_rgba(24,24,27,0.08)] sm:p-8 dark:border-slate-500/25 dark:bg-[#243144]/80 dark:shadow-[0_12px_40px_-20px_rgba(0,0,0,0.35)]">
      ...
      {err ? (
        <p className="mt-4 rounded-xl border border-red-200/80 bg-red-50/90 px-4 py-3 text-sm text-red-800 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200" role="alert">
          {err}
        </p>
      ) : null}
        ...
        <div className={`relative mt-8 ${pending ? "min-h-[260px]" : ""}`}>
          ...
          <div className="space-y-3">
            <label
              ...
              className={`flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 px-6 py-10 text-center transition ${
                selectedFile
                  ? "border border-emerald-200/90 bg-gradient-to-b from-emerald-50/95 to-white shadow-[0_20px_50px_-24px_rgba(16,185,129,0.35)] dark:border-emerald-700/50 dark:from-emerald-950/40 dark:to-slate-800/40 dark:shadow-[0_20px_50px_-24px_rgba(16,185,129,0.15)]"
                  : fileDrag
                    ? "border-dashed border-[#2563EB]/60 bg-[#2563EB]/[0.08] shadow-[0_20px_50px_-24px_rgba(37,99,235,0.2)] dark:border-sky-500/50 dark:bg-sky-500/10 dark:shadow-[0_20px_50px_-24px_rgba(56,189,248,0.15)]"
                    : "border-dashed border-zinc-200/95 bg-gradient-to-b from-zinc-50/80 to-white hover:border-[#2563EB]/45 hover:shadow-[0_20px_50px_-24px_rgba(37,99,235,0.12)] dark:border-slate-500/40 dark:from-slate-800/50 dark:to-slate-800/30 dark:hover:border-sky-500/40"
              }`}
            >
              {selectedFile ? (
                <>
                  <div className="relative mb-5 flex h-[5.5rem] w-[5.5rem] shrink-0 items-center justify-center rounded-2xl bg-gradient-to-b from-emerald-100/90 to-emerald-50 shadow-[0_12px_28px_-12px_rgba(16,185,129,0.45)] ring-1 ring-emerald-200/70 dark:from-emerald-900/50 dark:to-emerald-950/30 dark:ring-emerald-700/40">
                    <FileText className="h-[2.85rem] w-[2.85rem] text-emerald-800 dark:text-emerald-300" weight="duotone" aria-hidden />
                    <span className="absolute -bottom-1.5 -right-1.5 flex h-8 w-8 items-center justify-center rounded-full bg-emerald-600 text-white shadow-md ring-2 ring-white dark:ring-slate-800">
                      <CheckCircle className="h-4 w-4" weight="fill" aria-hidden />
                    </span>
                  </div>
                  <p className="text-sm font-semibold tracking-tight text-emerald-900 dark:text-emerald-200">Document added</p>
                  <p
                    className="mt-2 max-w-full truncate px-2 font-mono text-xs font-medium text-zinc-800 sm:text-sm dark:text-slate-200"
                    title={selectedFile.name}
                  >
                    {selectedFile.name}
                  </p>
                  <p className="mt-1 text-xs text-zinc-500 dark:text-slate-400">{formatFileSize(selectedFile.size)}</p>
                  <p className="mt-4 text-xs text-zinc-400 dark:text-slate-500">Click or drop to replace</p>
                </>
              ) : (
                <>
                  <div className="mb-5 flex h-[5.5rem] w-[5.5rem] shrink-0 items-center justify-center rounded-2xl bg-gradient-to-b from-white to-zinc-50/90 shadow-[0_12px_32px_-14px_rgba(24,24,27,0.12)] ring-1 ring-zinc-200/80 dark:from-slate-800/60 dark:to-slate-800/30 dark:ring-slate-600/40">
                    <FileText className="h-[2.85rem] w-[2.85rem] text-[#2563EB] dark:text-sky-400" weight="duotone" aria-hidden />
                  </div>
                  <span className="font-medium text-zinc-900 dark:text-slate-100">Drop a file or click to browse</span>
                  <span className="mt-1.5 max-w-sm text-sm leading-relaxed text-zinc-500 dark:text-slate-400">
              ...
              <>
                ...
                <div className="flex justify-center">
                  <button
                    type="button"
                    onClick={clearFile}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200/90 bg-white px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:border-red-200 hover:bg-red-50 hover:text-red-800 dark:border-slate-500/30 dark:bg-slate-800/50 dark:text-slate-300 dark:hover:border-red-800/50 dark:hover:bg-red-950/40 dark:hover:text-red-300"
                  >
          ...
          <div className="flex justify-end">
            <button
              ...
              className="inline-flex items-center justify-center rounded-xl bg-[#2563EB] px-6 py-3 text-sm font-semibold text-white shadow-[0_8px_24px_-8px_rgba(37,99,235,0.4)] transition hover:bg-[#1d4ed8] disabled:opacity-50"
            >
        ...
        <>
          <div
            className={`mt-6 flex items-start gap-3 rounded-xl border px-4 py-3 ${
              result.ok
                ? "border-emerald-200/90 bg-emerald-50/80 dark:border-emerald-800/50 dark:bg-emerald-950/35"
                : "border-amber-200/90 bg-amber-50/80 dark:border-amber-800/50 dark:bg-amber-950/30"
            }`}
          >
            <CheckCircle
              className={`mt-0.5 h-5 w-5 shrink-0 ${result.ok ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}
              weight="duotone"
              aria-hidden
            />
            <div>
          ...
          {parsedVacancy ? (
            ...
            <div className="mt-6 space-y-4">
              <div className="rounded-xl border border-zinc-200/80 bg-zinc-50/80 p-4 dark:border-slate-500/30 dark:bg-slate-800/40">
                <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-zinc-400 dark:text-slate-500">Technical details</p>
                <pre className="mt-2 max-h-[min(360px,50vh)] overflow-auto rounded-lg border border-zinc-200/80 bg-white p-3 font-mono text-xs leading-relaxed text-zinc-800 dark:border-slate-600/30 dark:bg-slate-900/50 dark:text-slate-300">
