    ...
    <div className="space-y-8">
      <SectionPanel>
        ...
        {listLoading ? (
          ...
          <ul className="mt-4 divide-y divide-zinc-200/80 rounded-xl border border-zinc-200/80 bg-white/80 dark:divide-slate-600/50 dark:border-slate-500/25 dark:bg-slate-800/40">
            {(users ?? []).map((row) => (
              <li key={row.uid} className="flex flex-wrap items-center justify-between gap-2 px-3 py-3 text-sm">
                <div className="min-w-0">
      ...
      {isAdmin ? (
        <SectionPanel>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-slate-400">
            Creates a sign-in for the same organization. Share the initial password securely; they can change it in{" "}
            <span className="font-medium text-zinc-800 dark:text-slate-200">Settings</span>.
          </p>
